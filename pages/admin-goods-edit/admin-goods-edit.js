    ...
    wx.uploadFile({
      ...
      success: (res) => {
        try {
          ...
          if (data.status === 200) {
            // 拼接完整URL，保证小程序中可显示
            const imageUrl = data.data.url.startsWith('http') ? data.data.url : app.globalData.serverUrl + data.data.url;
            if (type === 'main') {
              that.setData({ 'form.image': imageUrl });
            } else if (type === 'detail') {
              const key = 'detailImages[' + index + '].url';
              const uploadingKey = 'detailImages[' + index + '].uploading';
              that.setData({ [key]: imageUrl, [uploadingKey]: false });
